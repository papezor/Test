function loadAssets(){console.log('assets');jQuery.getScript(locAssets+"GeoReferenziazioneMunicipi.js", function(){loadPlessiAssets();});}
function loadPlessiAssets(){jQuery.getScript(locAssets+"AnagraficaPlessi.js", function(){checkParams();loadData();});}
...
if(contaNonPervTot <= 50) {
	  ...
	  var html= '<ul id="appoList" data-role="listview" style="padding-top:5px;margin-left:3px;margin-top:3px;width:99%;" >';
		...
		for (var x in plessi){
			...
			conta += sez.length;
			for (var y in sez){
				if (sez[y]["Municipio"] == filMun || filMun == 0){
					if ( dataSetSez[parseInt(sez[y]["Numero"])] == false){
			     	...
			     	//html += '<div data-corners="true" data-shadow="true" data-iconshadow="true" data-wrapperels="span" data-icon="null" data-iconpos="null" data-theme="c" class="ui-submit ui-btn ui-shadow ui-btn-corner-all ui-btn-up-c" aria-disabled="false" data-disabled="false"><span class="ui-btn-inner" style="color:#d81417;"><span class="ui-btn-text">Sez. '+sez+' - Municipio '+plesso[y]["Municipio"]+'</span></span><button type="submit" data-theme="c" class="ui-btn-hidden" data-disabled="false" onClick="'+func+'">'+sez+'</button></div>';
						html += '<li><a href="' + func + '">Sez. ' + sez[y]["Numero"] + ' - Municipio ' + sez[y]["Municipio"] + '</a></li>'; ;
						}
					}
				}
...
function addDataGraph(tit){
	var res2;
	jQuery.getJSON(locData+name+tipo+'P.json?nc='+Math.round(),
				{},
				function(result){
...
/* GET JSON PER LOCALE */
var e; var result;
jQuery.getJSON(locData+name+tipo+'.json?nc='+Math.round(),
				{},
...
function apriSezTab(x,y){
	...
	if (!document.getElementById('popDett' + sezione["Numero"])){
		  ...
		  html += '<div id="popDett' + sezione["Numero"] + '" class="contentFiltro ui-corner-all" style="height:auto !important;color:white;padding:5px 15px;">';
		 ...
		 // html+= '<a href="#mypanel" data-icon="plus" data-iconpos="notext">Add</a>';
		  ...
		  html += '<span style="color:' + color + ';font-size:1.2em;">Sezione ' + sezione["Numero"] + '</span>';
		  html += '</div>';
...
/* GET JSON PER LOCALE */
var cfg;
jQuery.getJSON(locData+"config.json?nc="+Math.round(),
				{},
				function(cfg){
